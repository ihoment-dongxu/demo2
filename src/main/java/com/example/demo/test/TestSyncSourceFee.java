 ...
 */
public class TestSyncSourceFee {
    ...
    private static void extracted() throws InterruptedException {
        ...
        Long endAt = 1722511234237L;
        Long syncStartAt;

        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

        OverseaWarehouseOutboundFeeSyncDTO dto = new OverseaWarehouseOutboundFeeSyncDTO();
        dto.setWarehouseArr(Collections.singletonList("GOODCANG"));
        ...
        do {
            ...
            Request request = new Request.Builder().url(OUTBOUND_SYNC_URL).method("POST", body).addHeader("Authorization", AUTHORIZATION).addHeader("Content-Type", "application/json").build();
            // 每次用这段时间请求同步
            try {
